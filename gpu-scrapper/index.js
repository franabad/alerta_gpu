...
module.exports = async function (context) {
  ...
  async function readData (lowestPrice) {
      ...
      if (lowestPrice) {
        if (article.promotionalPrice !== null) {
          if (lowestPrice > article.promotionalPrice) {
            ...
            const log = 'Correo enviado'
            context.log('Correo enviado')
            console.log(log)
            newPriceFound = true
          }
        } else {
    ...
    if (!newPriceFound) {
      const log = 'No se ha encontrado ningún precio más bajo\n'
      context.log(log)
      console.log(log)
    }
  }
  ...
  const text = readData.newPriceFound === false
    ? 'No se ha encontrado ningún precio más bajo'
    : `El precio más bajo actual es: ${readData.firstArticleLink} y su precio es: ${sendEmail.price}`
